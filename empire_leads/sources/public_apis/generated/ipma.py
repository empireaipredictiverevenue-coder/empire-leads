...
HTTPS:      True
CORS:       Unknown
Docs:       https://api.ipma.pt/open-data/
...
source returns []. To make it real:
  1. Read the docs at: https://api.ipma.pt/open-data/
...
logger = logging.getLogger("empire-leads.public_apis.ipma")
...
def _fetch(limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            logger.info("[ipma] docs URL returned non-JSON (HTML?) — stub returns []")
            return []
        ...
        logger.warning("[ipma] fetch failed: %s", e)
        return []
