...
HTTPS:      True
CORS:       Yes
Docs:       https://api.opensensemap.org/
...
source returns []. To make it real:
  1. Read the docs at: https://api.opensensemap.org/
...
logger = logging.getLogger("empire-leads.public_apis.opensensemap")
...
def _fetch(limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            logger.info("[opensensemap] docs URL returned non-JSON (HTML?) — stub returns []")
            return []
        ...
        logger.warning("[opensensemap] fetch failed: %s", e)
        return []
...
def _to_lead(item: dict, niche: str) -> Lead:
    ...
    return Lead(
        name=str(name)[:200],
        source="public_apis:opensensemap",
        niche=niche,
