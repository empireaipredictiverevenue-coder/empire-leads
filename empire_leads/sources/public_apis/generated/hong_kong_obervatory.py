...
HTTPS:      True
CORS:       Unknown
Docs:       https://www.hko.gov.hk/en/abouthko/opendata_intro.htm
...
source returns []. To make it real:
  1. Read the docs at: https://www.hko.gov.hk/en/abouthko/opendata_intro.htm
...
logger = logging.getLogger("empire-leads.public_apis.hong_kong_obervatory")

API_URL = "https://www.hko.gov.hk/en/abouthko/opendata_intro.htm"
TIMEOUT = 15
...
def _fetch(limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            logger.info("[hong_kong_obervatory] docs URL returned non-JSON (HTML?) — stub returns []")
            return []
        ...
        logger.warning("[hong_kong_obervatory] fetch failed: %s", e)
        return []
...
def _to_lead(item: dict, niche: str) -> Lead:
    ...
    name = (
        ...
        or "Hong Kong Obervatory"
    )
    return Lead(
        name=str(name)[:200],
        source="public_apis:hong_kong_obervatory",
        niche=niche,
